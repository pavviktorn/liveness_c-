...
from backbones.partbranch import PartBranch
import cv2
import numpy as np
...
class FASModel(nn.Module):
    def __init__(self, num_classes, backbone='resnet34'):
        ...
        if backbone == 'resnet18':
            ...
            self.branch3 = PartBranch(backbone='resnet18')
            self.branch4 = PartBranch(backbone='resnet18')
            self.branch5 = PartBranch(backbone='resnet18')
            self.branch6 = PartBranch(backbone='resnet18')
    ...
    def cal_feats(self, x):
        ...
        feat2 = self.branch3(xs[2])
        feat3 = self.branch3(xs[3])
        feat4 = self.branch4(xs[4])
        feat5 = self.branch5(xs[5])
        feat6 = self.branch5(xs[6])
        feat7 = self.branch6(xs[7])

        final_cls_feat = torch.cat([feat0, feat1, feat2, feat3, feat4, feat5, feat6, feat7], dim=1)
