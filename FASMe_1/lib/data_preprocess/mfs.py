...
from .cropface import get_align5p, get_cropped, get_parts
...
def get_mfs_results(mfs_fake, srcRgb, face_bbox, landmark, config, sliding_win=[256,256]):
    cropMfs_list = []
    bbox_list = []
    ...
    res = get_parts(mfs_fake, face_bbox, landmark)
    for idx in range(len(res)):
        cropMfs_list.append(res[idx])
...
def multi_scale_facial_swap(
    ...
    cropMfs_list, bbox_list = get_mfs_results(mfs_fake, srcRgb, face_bbox, landmark, config, sliding_win)
